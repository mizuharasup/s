...
getgenv().Config = {
    ["Feature Farm"] = {
        ["Auto All Melees"] = true,
        ["Saber"] = true,
        ["Soul Guitar"] = false,
        ["Cursed Dual Katana"] = false,
        ["Race V2"] = false,
        ["Stay Sea 2 Until Dark Fragment"] = false,
    },
...
Config = {
    Configuration = {
        HopWhenIdle = getgenv().Config["Auto Settings"]["Hop When Idle"],
        AutoHop = getgenv().Config["Auto Settings"]["Auto Hop"],
        FpsBoost = getgenv().Config["Auto Settings"]["FPS Boost"]
    },
    Items = {
        AutoFullyMelees = getgenv().Config["Feature Farm"]["Auto All Melees"],
        Saber = getgenv().Config["Feature Farm"]["Saber"],
        CursedDualKatana = getgenv().Config["Feature Farm"]["Cursed Dual Katana"],
        SoulGuitar = getgenv().Config["Feature Farm"]["Soul Guitar"],
        RaceV2 = getgenv().Config["Feature Farm"]["Race V2"]
    },
    Settings = {
        StayInSea2UntilHaveDarkFragments = getgenv().Config["Feature Farm"]["Stay Sea 2 Until Dark Fragment"]
    }
}
(function() 
    ...
    Interface.BlurManager = blurEffect
        ...
        alert("taphoamizu.com", "Endpoint reached")
...
if Config.Configuration.FpsBoost then
    spawn(function()
        ...
        -- Blox Fruits specific optimizations
 ...
 task.spawn(function()
    local success, CodesHttp = pcall(function()
        return game:HttpGet("https://raw.githubusercontent.com/mizuharasup/idk/refs/heads/main/idk2.lua")
    end)
    
    if success and CodesHttp then
        local CodesTable = CodesHttp:split(" ")
        if CodesTable and typeof(CodesTable) == 'table' then
    ...
    game.Players.LocalPlayer.Data.Level:GetPropertyChangedSignal('Value'):Connect(autoStats) 
        ...
        -- loadstring(game:HttpGet"https://github.com/taphoamizu.com-World/taphoamizu.com_BF/blob/main/InstanceDownloader.lua?raw=true")()
        
        local FolderPath = "taphoamizu.com/Blox_Fruit/Assets/"
    ...
    -- Tìm mob target để làm điểm neo
    local TAR = nil
    for _, Mon in Services.Workspace.Enemies:GetChildren() do 
        if Mon.Name == MobName and Mon:FindFirstChild("Humanoid") and 
           Mon:FindFirstChild("HumanoidRootPart") and Mon.Humanoid.Health > 0 then
            TAR = Mon
            break
        end
    end
    ...
    local V6 = TAR.HumanoidRootPart.CFrame
    ...
    if TAR then
        ...
        for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
            if broughtCount >= 10 then break end
            
            local Humanoid = v:FindFirstChild("Humanoid")
            local HRP = v:FindFirstChild("HumanoidRootPart") 
            if HRP and Humanoid and Humanoid.Health > 0 and
                v.Name == MobName and  -- Sử dụng MobName thay vì TAR.Name
                (HRP.Position - playerPos).Magnitude <= 500 and -- Sử dụng biến config
                v.Humanoid.MaxHealth < 100000 and not string.find(v.Name, "Boss")
            then
                local distanceToTarget = (HRP.Position - V6.Position).Magnitude
                
                if distanceToTarget <= 20 then
                    local lastHealth = v:GetAttribute("LastHealth")
                    local healthCheckTime = v:GetAttribute("HealthCheckTime")
                    local isBrought = v:GetAttribute("IsBrought")
                    
                    if not isBrought then
                        v:SetAttribute("IsBrought", true)
                        v:SetAttribute("LastHealth", Humanoid.Health)
                        v:SetAttribute("HealthCheckTime", currentTime)
                    else
                        local timeSinceCheck = currentTime - (healthCheckTime or 0)
                        
                        if timeSinceCheck >= 15 then
                            if Humanoid.Health == lastHealth or Humanoid.Health <= 0 then
                                -- Xóa mob hoàn toàn thay vì chỉ set health = 0
                                pcall(function()
                                    v:Destroy()
                                end)
                                continue
                ...
                -- Thêm attributes tương thích với code cũ
                v:SetAttribute("IsGrabbed", true)
                v:SetAttribute("MidPoint", V6)
                v:SetAttribute("OldPosition", v:GetAttribute("OldPosition") or HRP.Position)
            end
        end
    end 
    return true
        ...
        for _, Child in (MobTable) do
            ...
            elseif not NearbyHit then 
                ...
                if (os.time() - LastFound) > 200 then 
                    alert("taphoamizu.com", "Error while farming, rejoin")
    ...
    FunctionsHandler.RaidController:RegisterMethod("Start", function()
        ...
        if not CurrentIsland then 
            ...
            alert("taphoamizu.com", "Tween Paused")
            task.wait(.5)
    ...
    FunctionsHandler.Trevor:RegisterMethod("Start", function() 
        ...
        alert("[ taphoamizu.com ]", "Pulling fruit for trevor...")
    ...
    Hooks:RegisterNotifyListener("elite", function() 
        ...
        alert("[ taphoamizu.com ] ", "Elite defeated: ".. tostring(FunctionsHandler.Yama:Get("EliteCount") or "n/a"))
    end) 
    ...
    Window = fluent:CreateWindow{
        Title = "taphoamizu.com | Blox Fruit",
    ...
    Tabs.Home:CreateParagraph("Aligned Paragraph", {
        Title = "taphoamizu.com | BF Kaitun",
        Content = "by 1ltr3l - discord.gg/taphoamizu.com",
    ...
    print("RefreshTasksData called")
    if _G.Stop then
        print("Script stopped by _G.Stop")
        return
    end
    
    if ScriptStorage.Tools["Sweet Chalice"] and CurrentTask == "LevelFarm" then
        local PirateTask = FunctionsHandler["PirateRaid"]
        if PirateTask and PirateTask.Initalized then
            local PirateRefresh = PirateTask.Methods.Refresh:Call()
            if PirateRefresh then
                CurrentTask = "PirateRaid"
                ScriptStorage.Interface.SetText("DebugLine", "Priority: Pirate Raid")
                PirateTask.Methods.Start:Call(PirateRefresh)
                return
            end
        end
        
        local FruitTask = FunctionsHandler["CollectDrops"]
        if FruitTask and FruitTask.Initalized then
            local FruitRefresh = FruitTask.Methods.Refresh:Call()
            if FruitRefresh then
                CurrentTask = "CollectDrops"
                ScriptStorage.Interface.SetText("DebugLine", "Priority: Collect Fruit")
                FruitTask.Methods.Start:Call(FruitRefresh)
                return
            end
        end
        
        local EliteTask = FunctionsHandler["SpecialBossesTask"]
        if EliteTask and EliteTask.Initalized then
            local EliteRefresh = EliteTask.Methods.Refresh:Call()
            if EliteRefresh then
                CurrentTask = "SpecialBossesTask"
                ScriptStorage.Interface.SetText("DebugLine", "Priority: Elite Hunter")
                EliteTask.Methods.Start:Call(EliteRefresh)
                return
            end
        end
    end
    ...
    for _, TaskName in TasksOrder do
        ...
        getgenv().SendMessage = function(Message)
            ...
            if player and player.Character and player.Character:FindFirstChildOfClass("Humanoid") then
                animation.AnimationId = "http://www.roblox.com/asset/?id=buiquydiem07" .. tostring(Message)
        ...
        getgenv().PlayerAdded = function(plr)
            if plr.Character and not plr.Character:FindFirstChild("taphoamizu.com") and plr.Character:FindFirstChild("Humanoid") then
                Instance.new("Part", plr.Character).Name = "taphoamizu.com"
                plr.Character.Humanoid.AnimationPlayed:Connect(function(a)
                    local content = a.Animation.AnimationId
                    if string.match(content, "taphoamizu.com") then
                        local canret = false
                        ...
                        content = content:gsub("taphoamizu.com", "")
                        content = content:sub(2)
                        if content == getgenv().sicuda then
                        ...
                        elseif content == "Rejoin" .. getgenv().sicuda then
...
function closeSpinnerWindow()
    pcall(function()
        if LP and LP.PlayerGui and LP.PlayerGui:FindFirstChild('SpinnerWindow') then
            local spinnerWindow = LP.PlayerGui.SpinnerWindow
            if spinnerWindow:FindFirstChild('CloseButton') then
                firesignal(spinnerWindow.CloseButton.Activated)
                print("SpinnerWindow closed")
            end
        end
    end)
end
-- Add LPH_NO_VIRTUALIZE definition first
loadstring([[
    function LPH_NO_VIRTUALIZE(f) return f end;
]])();

-- Restart main loop with LPH_NO_VIRTUALIZE
_G.Stop = true
task.wait(2)
_G.Stop = false
task.spawn(LPH_NO_VIRTUALIZE(function()
    ...
    while task.wait() do
        if not _G.Stop then
            local success, error = pcall(RefreshTasksData)
            if not success then
...
spawn(function()
    ...
    function getTimeOfDay()
        ...
        local hours = math.floor(gameTime / 60)
        local minutes = math.floor(gameTime % 60)
        local seconds = math.floor((gameTime * 60) % 60)
